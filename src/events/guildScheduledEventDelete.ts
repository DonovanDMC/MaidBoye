...
export default new ClientEvent("guildScheduledEventDelete", async function guildScheduledEventDeleteEvent(event) {
    const events = await LogEvent.getType(event.guildID, LogEvents.SCHEDULED_EVENT_DELETE);
    if (events.length === 0) {
        return;
    }
