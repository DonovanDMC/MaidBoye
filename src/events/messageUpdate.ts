...
export default new ClientEvent("messageUpdate", async function debugEvent(message, oldMessage) {
    if (!("guildID" in message) || !message.guildID || oldMessage === null) return;

    if (message.content !== oldMessage.content) await db.redis
        .multi()
    ...
    if (oldMessage.flags !== message.flags) {
        if (!oldFlags.CROSSPOSTED && newFlags.CROSSPOSTED)  embeds.push(Util.makeEmbed(true, message.author)
            .setTitle("Message Updated")
            .setColor(Colors.gold)
        ...
        if (!oldFlags.SUPPRESS_EMBEDS && newFlags.SUPPRESS_EMBEDS)  embeds.push(Util.makeEmbed(true, message.author)
            .setTitle("Message Updated")
            .setColor(Colors.gold)
        ...
        if (!oldFlags.SOURCE_MESSAGE_DELETED && newFlags.SOURCE_MESSAGE_DELETED)  embeds.push(Util.makeEmbed(true, message.author)
            .setTitle("Message Updated")
            .setColor(Colors.gold)
        ...
        if (!oldFlags.HAS_THREAD && newFlags.HAS_THREAD)  embeds.push(Util.makeEmbed(true, message.author)
            .setTitle("Message Updated")
            .setColor(Colors.gold)
