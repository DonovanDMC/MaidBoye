...
export default new ClientEvent("threadDelete", async function(thread) {
	...
	const logEvents = await GuildConfig.getLogEvents(thread.guild.id, "threadDelete");
	for (const log of logEvents) {
		...
		if (thread.guild.permissionsOf(this.user.id).has("viewAuditLog")) {
			const audit = await BotFunctions.getAuditLogEntry(thread.guild, "THREAD_DELETE", (a) => a.targetID === thread.id);
