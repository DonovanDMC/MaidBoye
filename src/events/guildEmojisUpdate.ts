...
export default new ClientEvent("guildEmojisUpdate", async function guildEmojisUpdateEvent(guild, emojis, oldEmojis) {
    ...
    const addedEmojis = emojis.filter(e => !oldEmojis.find(oe => oe.id === e.id));
    const removedEmojis = oldEmojis.filter(e => !emojis.find(oe => oe.id === e.id));
    const updatedEmojis = emojis.filter(e => !addedEmojis.find(ae => ae.id === e.id) && JSON.stringify(e) !== JSON.stringify(oldEmojis.find(oe => oe.id === e.id)));
    ...
    if (eventsCreate.length > 0 && addedEmojis.length > 0) {
        ...
        if (guild instanceof Guild && guild.clientMember.permissions.has("VIEW_AUDIT_LOG")) {
            ...
            const ids = addedEmojis.map(e => e.id);
    ...
    if (eventsDelete.length > 0 && removedEmojis.length > 0) {
        ...
        if (guild instanceof Guild && guild.clientMember.permissions.has("VIEW_AUDIT_LOG")) {
            ...
            const ids = removedEmojis.map(e => e.id);
            const entry = auditLog.entries.find(e => e.targetID !== null && ids.includes(e.targetID));
            if (entry?.user && (entry.createdAt.getTime() + 5e3) > Date.now()) {
                const embed = Util.makeEmbed(true)
    ...
    if (eventsUpdate.length > 0 && updatedEmojis.length > 0) {
        ...
        if (guild instanceof Guild && guild.clientMember.permissions.has("VIEW_AUDIT_LOG")) {
            ...
            const ids = updatedEmojis.map(e => e.id);
            const entry = auditLog.entries.find(e => e.targetID !== null && ids.includes(e.targetID));
            if (entry?.user && (entry.createdAt.getTime() + 5e3) > Date.now()) {
                const embed = Util.makeEmbed(true)
