...
export default new ClientEvent("channelDelete", async function(channel) {
	...
	const logEvents = await GuildConfig.getLogEvents(channel.guild.id, "channelDelete");
	for (const log of logEvents) {
		...
		if (channel.guild.permissionsOf(this.user.id).has("viewAuditLog")) {
			const audit = await BotFunctions.getAuditLogEntry(channel.guild, "CHANNEL_DELETE", (a) => a.targetID === channel.id);
