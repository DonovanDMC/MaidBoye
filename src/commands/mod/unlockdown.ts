...
export default new Command("unlockdown")
	...
	.setExecutor(async function(msg) {
		...
		for (const ch of channels) {
			...
			await ch.editPermission(msg.channel.guild.id, BigInt(o[1]), BigInt(o[2]), Eris.Constants.PermissionOverwriteTypes.ROLE, `Unlockdown: ${msg.author.tag} (${msg.author.id}) -> ${reason ?? "None Provided"}`);
		}
