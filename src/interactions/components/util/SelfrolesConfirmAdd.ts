...
export default class SelfrolesConfirmAddComponent extends BaseComponent {
    action = "confirm-add";
    command = "selfroles";
    ...
    override async handleGuild(interaction: ComponentInteraction<ValidLocation.GUILD>, { role }: BaseState & { role: string; }) {
        ...
        if (!gConfig.selfroles.includes(role)) {
            gConfig.selfroles.push(role);
            await gConfig.edit({ selfroles: gConfig.selfroles });
        }

        return interaction.editParent(Util.replaceContent({ content: `Congrats, <@&${role}> is now self assignable` }));
    }
}
