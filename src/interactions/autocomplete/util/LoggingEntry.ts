...
export default class LoggingEntryAutocomplete extends BaseAutocomplete {
    ...
    override async handleGuild(interaction: AutocompleteInteraction<ValidLocation.GUILD>, focused: AnyAutocompleteFocus) {
        ...
        const choices = events.map(ev => ({ name: `${Util.readableConstant(LogEvents[ev.event])} in #${interaction.client.getChannel<AnyTextableGuildChannelWithoutThreads>(ev.channelID)?.name ?? ev.channelID}`, value: ev.id }));
