ALTER TABLE `blacklist`
	...
	CREATE COLUMN IF NOT EXISTS `user_id`         VARCHAR(21)  DEFAULT NULL,
	CREATE COLUMN IF NOT EXISTS `type`            TINYINT(1)   NOT NULL,
	CREATE COLUMN IF NOT EXISTS `reason`          TINYTEXT     DEFAULT NULL,
	CREATE COLUMN IF NOT EXISTS `notice_shown`    TINYINT(1)   NOT NULL DEFAULT 0,
	CREATE COLUMN IF NOT EXISTS `expire_time`     BIGINT(20)   NOT NULL DEFAULT 0,
	CREATE COLUMN IF NOT EXISTS `created_by`      VARCHAR(21)  NOT NULL,
	CREATE COLUMN IF NOT EXISTS `created_by_tag`  TINYTEXT     NOT NULL,
	CREATE COLUMN IF NOT EXISTS `created_at`      BIGINT(20)   NOT NULL,
	CREATE COLUMN IF NOT EXISTS `report`          TINYTEXT     DEFAULT NULL,
	-- Indexes
